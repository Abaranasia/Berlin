...
- [ ] 6.5 **(leave unchecked — human required)** Manual DAW-import gate: launch app (no file written) → click "Export MIDI..." (dialog seeded at `Documents/Berlin/berlin-export.mid`) → cancel (silent no-op) → export to a real path and open it in a DAW (120 BPM, 4/4, notes, 4 bars) → export to a read-only folder (dialog shown, no crash) → export during playback (no audible glitch).
