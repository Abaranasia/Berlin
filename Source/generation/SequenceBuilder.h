/*
  ==============================================================================
   ...
   from the juce_core-only harness. MUST take mode/pulses/rotation/
   stepProbability as explicit parameters (design.md Decision 2/V5 and
   probability-matrices Decision 7): this runs before any widget/processor
   member exists in the caller's ctor-init-list, so reading live state here
   would be undefined behaviour, not just bad style.
